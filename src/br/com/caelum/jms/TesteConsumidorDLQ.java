...
public class TesteConsumidorDLQ {
	...
	public static void main(String[] args) {
      ...
      //session -> para criar um consumer não podemos usar diretamente a conexão  precisamos de um objeto session esse obeto ssesion cria o cusumer . (A ssesion abstari a parte do trabalho trasacional e a parte de confirmação de recebimento da menssagem) 
       //CLIENT_ACKNOWLEDGE -> Não vamos tratar controle programático 
       //	Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
       	// SESSION_TRANSACTED -> para ter controle da transação  no controle programático 
    	Session session = connection.createSession(true, Session.SESSION_TRANSACTED);
       	...
       	consumer.setMessageListener(new MessageListener(){
       	    ...
       	    public void onMessage(Message message){
       	     try {
       	     //   message.acknowledge(); // fazendo programaticamente     (CLIENT_ACKNOWLEDGE)
       	    	session.commit(); // fazendo programaticamente   (true, Session.SESSION_TRANSACTED);
